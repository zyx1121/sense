...
- **分人** — 只要有語音在播（影片、podcast、通話），本機 diarizer（FluidAudio LS-EEND）就分辨講者；多講者內容的逐字稿自動分塊 — 一般內容標「**講者 A / B**」、會議標「**對方 A / B**」，單講者內容保留更有用的 app・標題來源。背景由 `gpt-5.4-mini` 讀近期輪替把標籤升級成角色與人名 —「**主持人 / 旁白 / 小明**」— 名字必須在逐字稿裡實際出現過才採用（不瞎猜）
