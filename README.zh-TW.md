...
- **分人** — 只要有語音在播（影片、podcast、通話），本機 diarizer（FluidAudio LS-EEND）就分辨講者；多講者內容的逐字稿自動分塊 — 一般內容標「**講者 A / B**」、會議標「**對方 A / B**」，單講者內容保留更有用的 app・標題來源。未知聲音刻意維持字母代號 — 命名是手動且明確的：輸入框打 **`/name A 王小明`**（或逐字稿右鍵 → 命名講者 A）綁定字母並在本機註冊聲紋（`~/.kilo/voices/`），之後跨影片、跨會議、跨重啟自動認人。認人先驗證再相信：具名配對要過 embedding 比對（該講者 ≥3 秒現場聲音 vs 庫存樣本）才放行，否則維持匿名 — 陌生聲音不會被誤掛成庫裡最像的人
