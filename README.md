...
- **Speaker separation** — whenever speech is playing (a video, a podcast, a call), an on-device diarizer (FluidAudio NVIDIA Streaming Sortformer; `--diarizer ls-eend` for >4-speaker content) tells voices apart; multi-speaker content gets per-speaker transcript blocks — **講者 A / B** normally, **對方 A / B** in meetings. Single-speaker content keeps the more useful app · title source. Unknown voices stay as letters by design — naming is manual and explicit: type **`/name A 王小明`** (or right-click the transcript → 命名講者 A) to bind a letter and enroll the voiceprint on-device (`~/.kilo/voices/`); from then on that voice is recognized automatically across videos, meetings, and restarts. Recognition is verified before it's trusted: a named match must pass an embedding check (≥3s of live audio vs the stored sample) or the speaker stays anonymous — unknown voices never get mislabeled as the nearest enrolled person
