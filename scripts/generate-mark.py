#!/usr/bin/env python3
"""KiloMark — zyx logo 單色 template PNG（狀態欄 + feed icon 用，可 tint）。
來源 scripts/zyx.svg。qlmanage 點陣黑 logo/白底 → 亮度反推 alpha → 輸出純白 + alpha mask
（SwiftUI/NSImage 當 template，染色靠 foregroundStyle / tint）。輸出 Resources/KiloMark.png（512 高清，resizable）。
...
OUT = os.path.join(HERE, "..", "Resources", "KiloMark.png")
