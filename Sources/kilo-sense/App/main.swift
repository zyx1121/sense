...
if CommandLine.arguments.contains("--diarize-selftest") {  // 分人核心驗證：音檔 → embedding 距離矩陣（headless）
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private var clusterer: SpeakerClusterer?   // --diarize 才建；nil = 不分人
    ...
    func applicationDidFinishLaunching(_: Notification) {
        ...
        if diarizationEnabled {  // 分人開：建 clusterer + 預熱 embedding 模型（須在 showSummaryWindow 前）
            let c = SpeakerClusterer(timeline: speakerTimeline)
            c.warmUp()
            clusterer = c
        }
        showOverlay()
    ...
    private func showSummaryWindow() {
        ...
        let controller = AgentController(store: transcript, agent: agent, metrics: metrics,
                                         polisher: polisher,
                                         speakers: speakerTimeline, clusterer: clusterer,
